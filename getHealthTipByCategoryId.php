    ...
    if(isset($postdata) && !empty($postdata)){
     //   print_r($postdata);
        ...
        //Sanitize.k
         ...
         $category = mysqli_real_escape_string($con, trim($getdata->cat));

    $row_count = mysqli_query($con,"SELECT * FROM health_tips WHERE tip_category_id = $category");
    $count = mysqli_num_rows($row_count);

    $sql = "SELECT * FROM health_tips WHERE tip_category_id = $category ORDER BY tip_id DESC LIMIT $limit OFFSET $offset";
